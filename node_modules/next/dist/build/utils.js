    ...
    const printFileTree = async ({ list, routerType })=>{
        ...
        filteredPages.forEach((item, i, arr)=>{
            ...
            if (item === "/_app" || item === "/_app.server") {
                ...
                if ((pageInfo == null ? void 0 : pageInfo.hasEmptyPrelude) || pageInfo.isDynamicAppRoute) {
                    symbol = "λ";
...
async function buildAppStaticPaths({ page, distDir, configFileName, generateParams, isrFlushToDisk, incrementalCacheHandlerPath, requestHeaders, maxMemoryCacheSize, fetchCacheKeyPrefix, staticGenerationAsyncStorage, serverHooks, ppr }) {
...
async function isPageStatic({ page, distDir, configFileName, runtimeEnvConfig, httpAgentOptions, locales, defaultLocale, parentId, pageRuntime, edgeInfo, pageType, originalAppPath, isrFlushToDisk, maxMemoryCacheSize, incrementalCacheHandlerPath, ppr }) {
    ...
    return isPageStaticSpan.traceAsyncFn(async ()=>{
        ...
        if (pageType === "app") {
            ...
            const generateParams = routeModule && _modulecompiled.AppRouteRouteModule.is(routeModule) ? [
                {
                    config: {
